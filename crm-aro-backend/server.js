...
//
// "Last contact" — max(Activity.createdAt) for the current holder against
// this lead, scoped to explicit contact types (call, meeting, followup,
// email, note). assignments[].lastActionAt was abandoned here because
// every rotation path creates a NEW slice with lastActionAt = now —
// rotated leads get a fresh "action clock" even when no agent contact
// happened (verified via audit; same root cause as the lastActivityTime
// pollution called out in the frontend rotation sweeper).
//
// status_change and reassign Activity types are auto-generated by system
// handlers and are excluded explicitly. They'd usually be excluded by
// the userId match anyway (auto-Activity userId = the admin who triggered
// the action, not the lead's agent), but the type filter is the durable
// safeguard.
//
// Fallback chain — primary lookup → current holder's slice.assignedAt
// (when they took the lead — an agent who's held it 14d without logging
// anything still buckets correctly) → lead.createdAt (legacy /
// pre-assignments docs).
//
...
app.get("/api/reports/overview/aging", auth, reportsAuth, async function(req, res) {
  try {
    ...
    var groups = await Lead.aggregate([
      ...
      { $addFields: {
          lastContact: { $ifNull: [
            { $arrayElemAt: ["$lastContactAct.t", 0] },
            { $ifNull: [
              { $let: {
                  vars: { cs: { $arrayElemAt: [
                    { $filter: {
