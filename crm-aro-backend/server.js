...
// TEMPORARY DIAGNOSTIC — REMOVE IN CLEANUP COMMIT
// ====================================================================
// GET  /api/admin/source-cleanup-preview
// POST /api/admin/source-cleanup-apply
//
// Two-step cleanup pair for legacy source values surfaced in the
// Leads filter dropdown after Slice A4-prep+. Preview enumerates every
// distinct source value NOT in the canonical SOURCES list, classifies
// each into one of:
//   • normalize → <canonical>     — when normalizeSource() resolves to
//                                   a SOURCES entry (e.g. "facebook" →
//                                   "Facebook", "Tiktok" → "TikTok")
//   • INVESTIGATE — DR mirror     — special-cased "Daily Request" so
//                                   admin can see whether each lead is
//                                   a legitimate DR mirror before any
//                                   write happens
//   • INVESTIGATE — no match      — anything else (typos, custom strings)
//
// Apply takes only the actions the admin explicitly confirms and
// validates each newValue against canonical SOURCES — preventing
// arbitrary retagging via this endpoint.
//
// Auth: admin / sales_admin only.
// Browser-console call:
//   fetch("https://crm-aro-backend-production.up.railway.app/api/admin/source-cleanup-preview", {
//     headers: { Authorization: "Bearer " + JSON.parse(localStorage.getItem("crm_aro_session")).token }
//   }).then(r => r.json()).then(d => console.log(JSON.stringify(d, null, 2)));
//
// REMOVE WHEN INVESTIGATION CLOSES.
// ====================================================================
app.get("/api/admin/source-cleanup-preview", auth, async function(req, res) {
  try {
    if (req.user.role !== "admin" && req.user.role !== "sales_admin") {
      return res.status(403).json({ error: "Admin only" });
    }
    ...
    for (var i = 0; i < dist.length; i++) {
      var currentValue = dist[i]._id;
      var leadCount = dist[i].count;
      // Skip empty/null sources (shouldn't exist post-A2 but defensive).
      if (currentValue == null || currentValue === "") continue;
      // Skip canonical values — not legacy.
      if (sourceSet[currentValue]) continue;

      var entry = { currentValue: currentValue, leadCount: leadCount };
      var normalized = normalizeSource(currentValue);

      if (normalized && sourceSet[normalized] && normalized !== currentValue) {
        // Has a clean canonical normalize target.
        entry.proposedAction = "normalize → " + normalized;
        entry.newValue = normalized;
        // Capped at 100 IDs to avoid mega-payloads on big cohorts; admin can
        // re-run with this as a starting point and check by phone/name if needed.
        var ids = await Lead.find({ source: currentValue }).select("_id").limit(100).lean();
        entry.leadIds = ids.map(function(d){ return String(d._id); });
      } else if (currentValue === "Daily Request") {
        // Legitimate DR mirrors live here (server.js:5251 hardcodes this on
        // the Lead.create payload when a DR transitions to EOI/DoneDeal/
        // MeetingDone). DON'T auto-normalize — surface samples + check
        // whether each has a corresponding DR row, so admin can decide
        // whether the right action is to retag, leave alone, or filter
        // from the dropdown UI without changing data.
        entry.proposedAction = "INVESTIGATE — DR-mirror leads (do not auto-normalize)";
        var samples = await Lead.find({ source: "Daily Request" })
          .sort({ createdAt: -1 })
          .limit(5)
          .select("_id name phone source createdAt agentId")
          .populate("agentId", "name")
          .lean();
        for (var j = 0; j < samples.length; j++) {
          var dr = await DailyRequest.findOne({ phone: samples[j].phone }).select("_id name").lean();
          samples[j]._hasDR = !!dr;
          samples[j]._drId = dr ? String(dr._id) : null;
        }
        entry.sampleLeads = samples.map(function(s){
          return {
            _id: String(s._id),
            name: s.name,
            phone: s.phone,
            ...
            agentName: s.agentId && s.agentId.name ? s.agentId.name : null,
            hasMatchingDR: s._hasDR,
            matchingDRId: s._drId
          };
        });
      } else {
        // Unrecognized value (typo, custom string, etc.). Surface samples
        // for manual review.
        entry.proposedAction = "INVESTIGATE — no canonical match";
        var others = await Lead.find({ source: currentValue })
          .sort({ createdAt: -1 })
          .limit(5)
          .select("_id name phone source createdAt agentId")
          .populate("agentId", "name")
          .lean();
        entry.sampleLeads = others.map(function(s){
          return {
            _id: String(s._id),
            name: s.name,
            phone: s.phone,
      ...
      legacyValues.push(entry);
      totalAffected += leadCount;
    }
...
app.post("/api/admin/source-cleanup-apply", auth, async function(req, res) {
  try {
    if (req.user.role !== "admin" && req.user.role !== "sales_admin") {
      return res.status(403).json({ error: "Admin only" });
    }
    var body = req.body || {};
    var confirmedActions = Array.isArray(body.confirmedActions) ? body.confirmedActions : [];
    if (!confirmedActions.length) {
      return res.status(400).json({ error: "confirmedActions required (array of {currentValue, newValue})" });
    }
    ...
    for (var i = 0; i < confirmedActions.length; i++) {
      var act = confirmedActions[i] || {};
      var currentValue = String(act.currentValue == null ? "" : act.currentValue);
      var newValue     = String(act.newValue     == null ? "" : act.newValue);

      if (!currentValue) {
        errors.push({ currentValue: currentValue, error: "currentValue required" });
        continue;
      }
      if (!newValue || !sourceSet[newValue]) {
        errors.push({ currentValue: currentValue, error: "newValue must be in canonical SOURCES" });
        continue;
      }
      if (currentValue === newValue) {
        errors.push({ currentValue: currentValue, error: "no-op (same value)" });
        continue;
      }

      try {
        // Single bulk write: $set the new source AND $push one history entry
        // onto every affected lead. All affected leads get the same timestamp
        // — they were retagged in the same admin action, which is the
        // correct semantic for the audit trail.
        var entry = historyEntry("source_changed",
          "Source: \"" + currentValue + "\" → \"" + newValue + "\" (bulk cleanup by " + actorName + ")",
          actorName, "");
        var result = await Lead.updateMany(
          { source: currentValue },
          { $set: { source: newValue }, $push: { history: entry } }
        );
        applied.push({
        ...
        errors.push({ currentValue: currentValue, error: errA.message });
      }
    }

    res.json({ applied: applied, errors: errors });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
