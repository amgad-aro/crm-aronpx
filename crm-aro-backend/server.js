...
// ===== TEMPORARY DEBUG — Reports KPI vs DealsPage reconciliation =====
// Diagnoses the revenue mismatch (DealsPage 165M vs Reports 151M) and the
// inflated pipeline value (5,378M). Read-only. Admin only.
//
// Call from browser console (after login):
//   fetch("/api/debug/reports-diagnosis?year=2026", {
//     headers: { Authorization: "Bearer " + JSON.parse(localStorage.crm_aro_session).token }
//   }).then(r=>r.json()).then(d=>console.log(JSON.stringify(d, null, 2)));
//
// REMOVE THIS ENDPOINT (and its header comment block) once both bugs are
// fixed and the fix is verified in production. Search for "DEBUG" to find
// it again.
app.get("/api/debug/reports-diagnosis", auth, async function(req, res) {
  ...
  try {
    ...
    var parseBudgetJs = function(b){ return parseFloat((b||"0").toString().replace(/,/g,"")) || 0; };
    var fields = "name status globalStatus dealStatus eoiStatus dealDate eoiDate budget projectWeight splitAgent2Id project createdAt updatedAt";

    // ---- DealsPage filter recreation (admin total uses RAW budget) ----
    // Mirrors src/App.js DealsPage `activeDeals` at line 6246-ish:
    //   (status==='DoneDeal' || globalStatus==='donedeal') && !archived
    //   && !(dealStatus==='Deal Cancelled' || status==='Deal Cancelled')
    var dealsPageDocs = await Lead.find({
      archived: { $ne: true },
      $and: [
    ...
    // DealsPage uses getDealDate(d) = dealDate || eoiDate || updatedAt || createdAt
    var dealsPageDateOf = function(d){
      if (d.dealDate) return new Date(d.dealDate);
      if (d.eoiDate)  return new Date(d.eoiDate);
      if (d.updatedAt) return new Date(d.updatedAt);
      return new Date(d.createdAt || 0);
    };
    var dealsPageInYear = dealsPageDocs.filter(function(d){
      var t = dealsPageDateOf(d).getTime();
      return t >= yearStart.getTime() && t < yearEnd.getTime();
    });
    var dealsPageRawTotal = dealsPageInYear.reduce(function(s,d){ return s + parseBudgetJs(d.budget); }, 0);

    // ---- Reports KPI filter recreation (WEIGHTED) ----
    // Mirrors GET /api/reports/overview/kpis Lead deals branch:
    //   archived: false, status: 'DoneDeal'
    //   dealAt = dealDate(parsed) || updatedAt
    //   weighted = budget × projectWeight × (splitAgent2Id ? 0.5 : 1)
    var reportsDocs = await Lead.find({ archived: false, status: "DoneDeal" }).select(fields).lean();
    var reportsDateOf = function(d){
      if (d.dealDate) {
        var t = new Date(d.dealDate).getTime();
        if (!isNaN(t)) return new Date(t);
      }
      return new Date(d.updatedAt || 0);
    };
    var reportsInYear = reportsDocs.filter(function(d){
      var t = reportsDateOf(d).getTime();
      return t >= yearStart.getTime() && t < yearEnd.getTime();
    });
    var reportsWeightedTotal = reportsInYear.reduce(function(s,d){
      var w = (typeof d.projectWeight === "number") ? d.projectWeight : 1;
      var sp = d.splitAgent2Id ? 0.5 : 1;
      return s + parseBudgetJs(d.budget) * w * sp;
    }, 0);
    var reportsRawTotal = reportsInYear.reduce(function(s,d){ return s + parseBudgetJs(d.budget); }, 0);

    // ---- Filter-set comparison (which docs are in one but not the other?) ----
    var dpIds  = new Set(dealsPageInYear.map(function(d){ return String(d._id); }));
    var rptIds = new Set(reportsInYear.map(function(d){ return String(d._id); }));
    var inDpOnly  = dealsPageInYear.filter(function(d){ return !rptIds.has(String(d._id)); });
    var inRptOnly = reportsInYear.filter(function(d){ return !dpIds.has(String(d._id)); });
    ...
    var summarize = function(d){
      return {
        ...
        name: d.name,
        status: d.status,
        globalStatus: d.globalStatus,
        dealStatus: d.dealStatus,
        eoiStatus: d.eoiStatus,
        dealDate: d.dealDate,
        eoiDate: d.eoiDate,
        budget: d.budget,
        budgetParsed: parseBudgetJs(d.budget),
        projectWeight: d.projectWeight,
        hasSplit: !!d.splitAgent2Id,
        project: d.project,
        updatedAt: d.updatedAt
      };
    };

    var topDealsContrib = reportsInYear.slice().sort(function(a,b){
      return parseBudgetJs(b.budget) - parseBudgetJs(a.budget);
    }).slice(0, 20).map(function(d){
      var raw = parseBudgetJs(d.budget);
      var w = (typeof d.projectWeight === "number") ? d.projectWeight : 1;
      var sp = d.splitAgent2Id ? 0.5 : 1;
      return {
        _id: String(d._id),
        ...
        projectWeight: w,
        hasSplit: !!d.splitAgent2Id,
        weighted: Math.round(raw * w * sp),
        delta: Math.round(raw - raw * w * sp)
      };
    });

    // ---- Pipeline diagnostic ----
    // Mirrors the kpis endpoint pipeline filter exactly:
    //   archived: false, status in [Potential, HotCase, MeetingDone, CallBack],
    //   globalStatus not in [donedeal, eoi]
    var pipelineDocs = await Lead.find({
      archived: false,
      status: { $in: ["Potential", "HotCase", "MeetingDone", "CallBack"] },
      globalStatus: { $nin: ["donedeal", "eoi"] }
    }).select(fields).lean();

    var pipelineTotal = pipelineDocs.reduce(function(s,d){ return s + parseBudgetJs(d.budget); }, 0);
    var dealCancelled = pipelineDocs.filter(function(d){ return d.dealStatus === "Deal Cancelled"; });
    var eoiCancelled  = pipelineDocs.filter(function(d){ return d.eoiStatus  === "EOI Cancelled"; });
    var anyCancelled  = pipelineDocs.filter(function(d){ return d.dealStatus === "Deal Cancelled" || d.eoiStatus === "EOI Cancelled"; });
    var cleanPipeline = pipelineDocs.filter(function(d){ return d.dealStatus !== "Deal Cancelled" && d.eoiStatus !== "EOI Cancelled"; });
    var sumBudget = function(arr){ return arr.reduce(function(s,d){ return s + parseBudgetJs(d.budget); }, 0); };

    // Budget format anomalies — only inspecting pipeline docs so we surface
    // bad data that's actually inflating the pipeline number.
    var budgetAnomalies = { empty: 0, hasComma: 0, hasNonDigitNonComma: 0, veryLarge_gt100M: 0, huge_gt1B: 0, sampleAnomalous: [] };
    pipelineDocs.forEach(function(d){
      var raw = (d.budget || "").toString().trim();
      if (!raw) { budgetAnomalies.empty++; return; }
      var hasComma = /,/.test(raw);
      var stripped = raw.replace(/,/g, "");
      var hasWeird = /[^0-9.\-]/.test(stripped);
      var parsed = parseBudgetJs(d.budget);
      if (hasComma) budgetAnomalies.hasComma++;
      if (hasWeird) budgetAnomalies.hasNonDigitNonComma++;
      if (parsed > 100000000) budgetAnomalies.veryLarge_gt100M++;
      if (parsed > 1000000000) budgetAnomalies.huge_gt1B++;
      if (hasWeird || parsed > 100000000) {
        if (budgetAnomalies.sampleAnomalous.length < 25) {
          budgetAnomalies.sampleAnomalous.push({
            _id: String(d._id), name: d.name,
            budgetRaw: d.budget, parsed: parsed,
            hasComma: hasComma, hasNonDigitNonComma: hasWeird,
            status: d.status, dealStatus: d.dealStatus, eoiStatus: d.eoiStatus
          });
        }
      }
    });

    var top10Pipeline = pipelineDocs.slice().sort(function(a,b){
      return parseBudgetJs(b.budget) - parseBudgetJs(a.budget);
    }).slice(0, 10).map(summarize);

    res.json({
      year: year,
      yearRange: { from: yearStart.toISOString(), to: yearEnd.toISOString() },

      dealsPage: {
        formula: "RAW: parseBudget(d.budget). Filter: (status==='DoneDeal' || globalStatus==='donedeal') && !archived && !(dealStatus==='Deal Cancelled' || status==='Deal Cancelled'). Date: dealDate || eoiDate || updatedAt || createdAt.",
        countAllTime: dealsPageDocs.length,
        countInYear: dealsPageInYear.length,
        totalRawEGP: Math.round(dealsPageRawTotal),
        totalRawM: Number((dealsPageRawTotal/1e6).toFixed(2))
      },

      reports: {
        formula: "WEIGHTED: budget × projectWeight × (splitAgent2Id ? 0.5 : 1). Filter: archived=false, status='DoneDeal'. Date: dealDate(parsed) || updatedAt.",
        countAllTime: reportsDocs.length,
        countInYear: reportsInYear.length,
        totalWeightedEGP: Math.round(reportsWeightedTotal),
        totalWeightedM: Number((reportsWeightedTotal/1e6).toFixed(2)),
        totalRawOnSameSetEGP: Math.round(reportsRawTotal),
        totalRawOnSameSetM: Number((reportsRawTotal/1e6).toFixed(2)),
        deltaWeightingOnlyEGP: Math.round(reportsRawTotal - reportsWeightedTotal),
        deltaWeightingOnlyM: Number(((reportsRawTotal - reportsWeightedTotal)/1e6).toFixed(2)),
        topDealsByRawBudget: topDealsContrib
      },

      comparison: {
        dealsPageMinusReports_M: Number(((dealsPageRawTotal - reportsWeightedTotal)/1e6).toFixed(2)),
        explanation: "Total diff = filter-set delta + math delta. inDpOnly/inRptOnly isolate filter-set delta. reports.deltaWeightingOnly isolates math delta on the shared set.",
        inDpOnly_count: inDpOnly.length,
        inDpOnly_sumBudget: Math.round(inDpOnly.reduce(function(s,d){ return s + parseBudgetJs(d.budget); }, 0)),
        inDealsPageButNotReports: inDpOnly.map(summarize).slice(0, 25),
        inRptOnly_count: inRptOnly.length,
        inRptOnly_sumBudget: Math.round(inRptOnly.reduce(function(s,d){ return s + parseBudgetJs(d.budget); }, 0)),
        inReportsButNotDealsPage: inRptOnly.map(summarize).slice(0, 25)
      },

      pipeline: {
        filter: "archived=false, status in [Potential, HotCase, MeetingDone, CallBack], globalStatus not in [donedeal, eoi]",
        currentTotalEGP: Math.round(pipelineTotal),
        currentTotalM: Number((pipelineTotal/1e6).toFixed(1)),
        currentCount: pipelineDocs.length,
        buckets: {
          dealCancelled_count: dealCancelled.length,
          dealCancelled_sumM: Number((sumBudget(dealCancelled)/1e6).toFixed(1)),
          eoiCancelled_count: eoiCancelled.length,
          eoiCancelled_sumM: Number((sumBudget(eoiCancelled)/1e6).toFixed(1)),
          anyCancelled_count: anyCancelled.length,
          anyCancelled_sumM: Number((sumBudget(anyCancelled)/1e6).toFixed(1)),
          cleanPipeline_count: cleanPipeline.length,
          cleanPipeline_sumM: Number((sumBudget(cleanPipeline)/1e6).toFixed(1))
        },
        top10ByBudget: top10Pipeline,
        budgetAnomalies: budgetAnomalies
      }
    });
  } catch(e) {
    console.error("[debug/reports-diagnosis]", e && e.message);
    res.status(500).json({ error: e && e.message ? e.message : "diagnosis_failed" });
  }
});
// ===== END TEMPORARY DEBUG =====
