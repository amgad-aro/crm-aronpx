...
// =============================================================================
// TEMPORARY DIAGNOSTIC — aging whitelist redistribution audit (Phase 3 cleanup
// Priority 1). Read-only, admin-only. Returns the bucket redistribution that
// would happen on /api/reports/overview/aging if `callback_scheduled` is
// dropped from the gated history-event whitelist. Sized so we can decide
// whether the residual is worth fixing before touching the live endpoint.
// REMOVE IN CLEANUP COMMIT — search for "aging-whitelist-diagnose" to find
// every reference. The whole endpoint is a single self-contained block.
// =============================================================================
app.get("/api/admin/aging-whitelist-diagnose", auth, async function(req, res) {
  try {
    if (req.user.role !== "admin" && req.user.role !== "sales_admin") {
      return res.status(403).json({ error: "Admin only" });
    }
    ...
    // Mirror /api/reports/overview/aging exclusions exactly. No team / source
    // filter — diagnostic measures org-wide impact.
    var match = {
      archived: false,
    ...
    // Single scan: compute lastContact for BOTH whitelists per doc, bucket
    // each, project lean rows. JS post-processing builds the summary blocks.
    var rows = await Lead.aggregate([
      { $match: match },
      { $lookup: {
          from: "activities",
      ...
      { $addFields: {
          lastContactCur:  { $ifNull: [
            { $max: ["$historyContactMaxCur",  "$activityContactMax"] },
            { $ifNull: ["$curSliceAssignedAt", "$createdAt"] }
          ]},
          lastContactProp: { $ifNull: [
            { $max: ["$historyContactMaxProp", "$activityContactMax"] },
            { $ifNull: ["$curSliceAssignedAt", "$createdAt"] }
          ]}
      }},
      { $addFields: {
          ageDaysCur:  { $divide: [{ $subtract: [nowDate, "$lastContactCur"]  }, DAY_MS] },
          ageDaysProp: { $divide: [{ $subtract: [nowDate, "$lastContactProp"] }, DAY_MS] }
      }},
      { $project: {
          _id: 1, name: 1, status: 1, agentId: 1,
          ageDaysCur:  { $round: ["$ageDaysCur",  1] },
          ageDaysProp: { $round: ["$ageDaysProp", 1] }
      }}
    ]);

    function bucketOf(age) {
      if (age >= 14) return "over14";
      if (age >= 7)  return "days7to14";
      if (age >= 3)  return "days3to7";
      return "under3";
    }

    var curBuckets  = { under3: 0, days3to7: 0, days7to14: 0, over14: 0 };
    var propBuckets = { under3: 0, days3to7: 0, days7to14: 0, over14: 0 };
    var movers = [];

    rows.forEach(function(r){
      var bc = bucketOf(r.ageDaysCur);
      var bp = bucketOf(r.ageDaysProp);
      curBuckets[bc]++;
      propBuckets[bp]++;
      if (bc !== bp) {
        movers.push({
          ...
          wasBucket: bc, wasAge: r.ageDaysCur,
          nowBucket: bp, nowAge: r.ageDaysProp
        });
      }
    });

    var deltas = {
      under3:    propBuckets.under3    - curBuckets.under3,
      days3to7:  propBuckets.days3to7  - curBuckets.days3to7,
      days7to14: propBuckets.days7to14 - curBuckets.days7to14,
      over14:    propBuckets.over14    - curBuckets.over14
    };

    var byDirection = {};
    movers.forEach(function(m){
      var k = m.wasBucket + "->" + m.nowBucket;
      byDirection[k] = (byDirection[k] || 0) + 1;
    });

    // Top 10 movers INTO over14 / days7to14, sorted by new age desc.
    var intoLate = movers
      .filter(function(m){ return m.nowBucket === "over14" || m.nowBucket === "days7to14"; })
      .sort(function(a, b){ return b.nowAge - a.nowAge; })
      .slice(0, 10);

    // Resolve agent names for the top-10 list only (small batch).
    var agentIds = [];
    intoLate.forEach(function(m){ if (m.agentId) agentIds.push(new mongoose.Types.ObjectId(m.agentId)); });
    var agents = agentIds.length ? await User.find({ _id: { $in: agentIds }}).select({ name: 1 }).lean() : [];
    var agentNameById = {};
    agents.forEach(function(u){ agentNameById[String(u._id)] = u.name || "(unknown)"; });
    intoLate.forEach(function(m){ m.agentName = m.agentId ? (agentNameById[m.agentId] || "(unknown)") : "Unassigned"; });

    // Population stats — gauges how rescheduling-heavy this CRM is.
    var cbStats = await Lead.aggregate([
      { $match: { archived: false, source: { $ne: "Daily Request" }}},
      { $project: {
          cbCount: { $size: { $filter: {
            input: { $ifNull: ["$history", []] },
            as: "h",
      ...
      { $group: {
          _id: null,
          totalLeads:       { $sum: 1 },
          leadsWithAnyCb:   { $sum: { $cond: [{ $gt:  ["$cbCount", 0]  }, 1, 0] }},
          leadsWith5Plus:   { $sum: { $cond: [{ $gte: ["$cbCount", 5]  }, 1, 0] }},
          leadsWith10Plus:  { $sum: { $cond: [{ $gte: ["$cbCount", 10] }, 1, 0] }},
          totalCbEvents:    { $sum: "$cbCount" },
          maxCbOnOneLead:   { $max: "$cbCount" }
      }}
    ]);
    var cb = cbStats[0] || { totalLeads: 0, leadsWithAnyCb: 0, leadsWith5Plus: 0, leadsWith10Plus: 0, totalCbEvents: 0, maxCbOnOneLead: 0 };
    function pctOf(n, d) { return d > 0 ? Math.round(1000 * n / d) / 10 : 0; }

    res.json({
      current: {
        whitelist: ["status_changed", "feedback_added", "callback_scheduled"],
        buckets: curBuckets,
        total: rows.length
      },
      proposed: {
        whitelist: ["status_changed", "feedback_added"],
        buckets: propBuckets,
        total: rows.length,
        deltas: deltas
      },
      ...
      topMoversIntoLate: intoLate,
      callbackPollution: {
        totalLeads:        cb.totalLeads,
        leadsWithAnyCb:    cb.leadsWithAnyCb,
        leadsWithAnyCbPct: pctOf(cb.leadsWithAnyCb, cb.totalLeads),
        leadsWith5Plus:    cb.leadsWith5Plus,
        leadsWith10Plus:   cb.leadsWith10Plus,
        totalCbEvents:     cb.totalCbEvents,
        maxCbOnOneLead:    cb.maxCbOnOneLead
      },
      asOf: nowDate.toISOString()
    });
  } catch (e) {
    console.error("[admin/aging-whitelist-diagnose]", e && e.message);
