  ...
  try {
    ...
    var drAggP = includeDR ? DailyRequest.aggregate([
      ...
      { $addFields: {
          ...
          // DR-side state-only contacted check. DRs have no history[] and
          // no Activity records (Activities reference Lead._id, not DR._id),
          // so the Lead-side UNION pattern can't apply. We keep only the
          // two state checks that represent actual customer-side evidence:
          //   - status != NewLead    : real status progression (clause 1)
          //   - lastFeedback non-empty: real customer feedback (clause 3)
          // Dropped 2026-05-05 (diagnostic /api/admin/funnel-dr-diagnose):
          //   - notes non-empty       (clause 2): auto-populated default;
          //                            in production all 67 NewLead DRs had
          //                            notes != "" without any contact
          //   - callbackTime non-empty (clause 4): same auto-populated
          //                            default — fired on every NewLead DR
          isContactedRaw: { $or: [
            { $ne: ["$status", "NewLead"] },
...
// =============================================================================
app.get("/api/admin/callback-gaming-diagnose", auth, async function(req, res) {
  try {
    if (req.user.role !== "admin" && req.user.role !== "sales_admin") {
      return res.status(403).json({ error: "Admin only" });
    }
    ...
    var DAY_MS = 86400000;
    var nowDate = new Date();
    var FOURTEEN_DAYS_AGO = new Date(nowDate.getTime() - 14 * DAY_MS);

    // Match: Lead-only (DRs lack history[]/Activity link), CallBack status,
    // not archived. Snapshot — no date scoping.
    var match = {
      archived: false,
      status: "CallBack",
    ...
    var leads = await Lead.aggregate([
      { $match: match },
      // Latest contact-type Activity. Sort DESC + limit:1 → ships at most
      // one row. If lastContactAct.t < 14d-ago (or empty), then no activity
      // within 14d (since this IS the most recent). Single lookup serves
      // both the recency check + the daysSinceLastProgress baseline.
      { $lookup: {
          from: "activities",
          let: { lid: "$_id" },
          pipeline: [
      ...
      { $addFields: {
          // Lifetime count of callback_scheduled events.
          callbackCount: { $size: { $filter: {
            input: { $ifNull: ["$history", []] },
            as: "h",
          ...
          lastContactActAt: { $arrayElemAt: ["$lastContactAct.t", 0] },
          // Budget parsed defensively for the value-at-risk sum.
          budgetNum: { $convert: {
            input: { $replaceAll: { input: { $toString: { $ifNull: ["$budget", "0"] }}, find: ",", replacement: "" }},
            to: "double", onError: 0, onNull: 0
          }}
      ...
      { $addFields: {
          // hasRecentContactActivity — is the latest contact activity
          // within 14d? Two-step guard: only compare if any exists.
          hasRecentContactActivity: { $and: [
            { $gt: [{ $size: "$lastContactAct" }, 0] },
      ...
      { $project: {
          ...
          callbackCount: 1, isGamed: 1,
          isGamedAtMin5: 1, isGamedAtMin10: 1, isGamedAtMin15: 1, isGamedAtMin20: 1,
          lastProgressAt: 1, budgetNum: 1
      }}
    ]);
    ...
    function bucketOf(c) {
      ...
      if (c <= 29) return "20-29";
      return "30+";
    }
    function pct(num, denom) { return denom > 0 ? Math.round(1000 * num / denom) / 10 : 0; }

    var distribution = { "0-2": 0, "3-5": 0, "6-9": 0, "10-19": 0, "20-29": 0, "30+": 0 };
    var thresholdHits = { atMin5: 0, atMin10: 0, atMin15: 0, atMin20: 0 };
    var totalEstimatedValue = 0;
    var gamedDocs = [];
    var byAgentMap = {};

    leads.forEach(function(l){
      distribution[bucketOf(l.callbackCount)]++;
      if (l.isGamedAtMin5)  thresholdHits.atMin5++;
      if (l.isGamedAtMin10) thresholdHits.atMin10++;
      if (l.isGamedAtMin15) thresholdHits.atMin15++;
      if (l.isGamedAtMin20) thresholdHits.atMin20++;

      if (l.isGamed) {
        gamedDocs.push(l);
        totalEstimatedValue += Number(l.budgetNum) || 0;
      }

      var agentKey = String(l.agentId || "unassigned");
      if (!byAgentMap[agentKey]) {
        byAgentMap[agentKey] = {
      ...
      byAgentMap[agentKey].totalCallbackCount++;
      if (l.isGamed) {
        byAgentMap[agentKey].gamedCount++;
        if (!byAgentMap[agentKey].topLead || l.callbackCount > byAgentMap[agentKey].topLead.callbackCount) {
          byAgentMap[agentKey].topLead = {
            _id: String(l._id),
            name: l.name || "(no name)",
            callbackCount: l.callbackCount
          };
        }
      }
    });

    // Resolve user names/roles for any agent that holds at least one
    // CallBack lead (covers both gamed-holders + the "totalCallbackCount"
    // breakdown rows). Single batched query.
    var allAgentIds = Object.keys(byAgentMap)
      .filter(function(k){ return k !== "unassigned"; })
      .map(function(k){ return new mongoose.Types.ObjectId(k); });
    var users = allAgentIds.length
      ? await User.find({ _id: { $in: allAgentIds }}).select({ name: 1, role: 1 }).lean()
      : [];
    var userMap = {};
    users.forEach(function(u){ userMap[String(u._id)] = u; });

    var byAgent = Object.values(byAgentMap)
      .filter(function(a){ return a.gamedCount > 0; })
      .map(function(a){
        var u = a.agentId ? userMap[String(a.agentId)] : null;
        return {
          agentId: a.agentId ? String(a.agentId) : null,
          agentName: u ? (u.name || "(unknown)") : "Unassigned",
          role: u ? (u.role || null) : null,
          gamedCount: a.gamedCount,
          totalCallbackCount: a.totalCallbackCount,
          gamingRate: pct(a.gamedCount, a.totalCallbackCount),
          topLeadId: a.topLead ? a.topLead._id : null,
          topLeadName: a.topLead ? a.topLead.name : null,
          topLeadReschedules: a.topLead ? a.topLead.callbackCount : 0
        };
      })
      .sort(function(a, b){ return b.gamedCount - a.gamedCount; });

    // Top gamed leads — sort by rescheduleCount desc, take 20.
    gamedDocs.sort(function(a, b){ return b.callbackCount - a.callbackCount; });
    var topGamedLeads = gamedDocs.slice(0, 20).map(function(l){
      var anchor = l.lastProgressAt || l.createdAt;
      var ageMs = nowDate.getTime() - new Date(anchor).getTime();
      var u = l.agentId ? userMap[String(l.agentId)] : null;
      return {
        leadId: String(l._id),
        name: l.name || "(no name)",
        status: "CallBack",
        agentName: u ? (u.name || "(unknown)") : "Unassigned",
        rescheduleCount: l.callbackCount,
        daysSinceLastProgress: Math.round((ageMs / DAY_MS) * 10) / 10,
        budget: Math.round(Number(l.budgetNum) || 0),
        createdAt: l.createdAt
      };
    });

    var totalCallbackLeads = leads.length;
    var affectedAgents = Object.values(byAgentMap).filter(function(a){ return a.gamedCount > 0; }).length;
    var gamedCount = gamedDocs.length;

    res.json({
      scope: {
        totalCallbackLeads: totalCallbackLeads,
        thresholds: { rescheduleCountMin: 10, noProgressWindowDays: 14 }
      },
      callbackCountDistribution: Object.keys(distribution).map(function(k){
        return { bucket: k, count: distribution[k] };
      }),
      thresholdSensitivity: {
        atMin5:  { gamedCount: thresholdHits.atMin5,  pctOfCallback: pct(thresholdHits.atMin5,  totalCallbackLeads) },
        atMin10: { gamedCount: thresholdHits.atMin10, pctOfCallback: pct(thresholdHits.atMin10, totalCallbackLeads) },
        atMin15: { gamedCount: thresholdHits.atMin15, pctOfCallback: pct(thresholdHits.atMin15, totalCallbackLeads) },
        atMin20: { gamedCount: thresholdHits.atMin20, pctOfCallback: pct(thresholdHits.atMin20, totalCallbackLeads) }
      },
      summary: {
        gamedLeads: gamedCount,
        gamedPct: pct(gamedCount, totalCallbackLeads),
        affectedAgents: affectedAgents,
        totalEstimatedValue: Math.round(totalEstimatedValue)
      },
      byAgent: byAgent,
      topGamedLeads: topGamedLeads,
      asOf: nowDate.toISOString()
    });
  } catch (e) {
    console.error("[admin/callback-gaming-diagnose]", e && e.message);
