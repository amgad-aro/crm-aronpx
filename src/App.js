  ...
  return <div style={{ height:64, background:"#fff", borderBottom:"1px solid #E8ECF1", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 16px", position:"sticky", top:0, zIndex:100, gap:10 }}>
    ...
    <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
      ...
      {(p.isAdmin||p.cu&&p.cu.role==="sales_admin")&&<div ref={dealNotifRef} style={{ position:"relative" }}>
        ...
        {p.showDealNotif&&<div style={{ position:"absolute", top:44, right:0, width:310, background:"#fff", borderRadius:14, boxShadow:"0 12px 48px rgba(0,0,0,0.15)", border:"1px solid #E8ECF1", zIndex:200, maxHeight:400, overflowY:"auto" }}>
      ...
      {results.map(function(l){
        ...
        return <div key={gid(l)} onClick={function(){p.onSelect(l);setShow(false);setQ("");}} style={{ padding:"12px 14px", borderRadius:12, border:"1px solid #E8ECF1", marginBottom:8, cursor:"pointer", background:"#FAFBFC" }}>
          ...
          <div style={{ display:"flex", gap:8, marginTop:8 }}>
            <a href={"tel:"+l.phone} onClick={function(e){e.stopPropagation();}} style={{ flex:1, padding:"6px", borderRadius:8, background:"#DCFCE7", color:"#15803D", fontSize:12, fontWeight:600, textDecoration:"none", display:"flex", alignItems:"center", justifyContent:"center", gap:4 }}>📞 Call</a>
                ...
                return <tr key={lid} onClick={function(){setSelected(lead);}} style={{ borderBottom:"1px solid #F1F5F9", cursor:"pointer", background:isSel?"#EFF6FF":isVIP?"#FFFBEB":isChk?"#F0FDF4":"transparent", transition:"background 0.12s", borderRight:isVIP?"3px solid #F59E0B":"3px solid transparent" }}>
                  ...
                  <td style={{ padding:"10px 12px", whiteSpace:"nowrap", textAlign:"left" }}>
                    ...
                    <div style={{ display:"flex", gap:6, marginTop:3, justifyContent:"flex-start" }}>
                      <a href={"tel:"+lead.phone} onClick={function(e){e.stopPropagation();}} style={{ fontSize:p.isMobile?10:12, color:C.success, textDecoration:"none", display:"flex", alignItems:"center", gap:3, padding:p.isMobile?"0":"2px 6px", borderRadius:6, background:p.isMobile?"transparent":"#DCFCE7" }}><Phone size={p.isMobile?9:12}/>{!p.isMobile&&" "+t.call}</a>
...
var DashboardPage = function(p) {
  ...
  return <div style={{ padding:"18px 16px 40px" }}>
    ...
    {isAdmin&&<div style={{ display:"flex", gap:10, marginBottom:22, flexWrap:"wrap" }}>
      ...
      {(function(){var rots=[];try{rots=JSON.parse(localStorage.getItem("crm_rot_notifs")||"[]");}catch(e){}var todayRots=rots.filter(function(r){return r.time&&(Date.now()-new Date(r.time).getTime())<24*60*60*1000;});return todayRots.length>0?<StatCard icon={RotateCcw} label="Rotations Today" value={todayRots.length+""} c={C.warning}/>:null;})()}
    </div>}
              ...
              {filtered.map(function(r){
                ...
                return <tr key={rid} onClick={function(){setSelected(r);}} style={{ borderBottom:"1px solid #F1F5F9", cursor:"pointer", background:isChk?"#EFF6FF":isSel?"#EFF6FF":"transparent", borderRight:"3px solid "+(isSel?C.accent:"transparent") }}>
                  ...
                  <td style={{ padding:"10px 12px", fontSize:12, direction:"ltr" }}>
                    ...
                    <div style={{ display:"flex", gap:6, marginTop:3 }}>
                      <a href={"tel:"+r.phone} onClick={function(e){e.stopPropagation();}} style={{ fontSize:p.isMobile?10:12, color:C.success, textDecoration:"none", display:"flex", alignItems:"center", gap:3, padding:p.isMobile?"0":"2px 6px", borderRadius:6, background:p.isMobile?"transparent":"#DCFCE7" }}><Phone size={p.isMobile?9:12}/>{!p.isMobile&&" Call"}</a>
