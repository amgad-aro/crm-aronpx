...
var LeadForm = function(p) {
  ...
  var submit = async function() {
    ...
    setSaving(true);
    try {
      var payload = Object.assign({}, form, { source: isReq?"Daily Request":form.source, agentId: form.agentId||(salesUsers[0]?gid(salesUsers[0]):p.cu.id), status: form.status==="NewLead"?"Potential":form.status||"Potential" });
      var result = p.editId
