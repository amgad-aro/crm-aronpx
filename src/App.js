...
var DailyRequestsPage = function(p) {
  ...
  return <div style={{ padding:"18px 16px 40px" }}>
    ...
    <div style={{ display:"flex", gap:8, marginBottom:12, flexWrap:"wrap", alignItems:"center" }}>
      ...
      {p.cu.role==="admin"&&<Btn outline onClick={function(){if(selected2.length>0)setShowBulk(true);}} disabled={selected2.length===0} style={{ padding:"7px 11px", fontSize:12, color:selected2.length>0?C.info:"#94A3B8", borderColor:selected2.length>0?C.info:"#E2E8F0", opacity:selected2.length===0?0.5:1 }}><RotateCcw size={13}/> Bulk Reassign {selected2.length>0?"("+selected2.length+")":""}</Btn>}
